...
from adavoc_package.data import VOC_ROOT, VOCDetection, BaseTransform
# from data import VOC_ROOT, VOCAnnotationTransform, VOCDetection, BaseTransform
from adavoc_package.data import VOC_CLASSES as labelmap
import torch.utils.data as data
from PIL import Image
from adavoc_package.ssd import build_ssd
...
from adavoc_package.RFB.utils.box_utils import match, log_sum_exp
...
def test_net(netG, img_path):
    ...
    im = PIL2tensor(img_pil).unsqueeze(0).cuda()
    ...
    print('cleaned image shape', x.shape)


    img_name = os.path.basename(img_path)[:-4]  # peacock
    output_img_dir = os.path.dirname(img_path)
    os.makedirs(f'{output_img_dir}_adavoc', exist_ok=True)
    save_image(x[0], f'{output_img_dir}_adavoc/{img_name}_adavoc.png')
...
def cleaning(img_path):
    ...
    from adavoc_package.RFB.models.RFB_Net_vgg import build_net
    img_dim = 300
    num_classes = 21
    ...
    for mm in range(len(save_path_list)):
        ...
        test_net(netG, img_path)

def cleaning_folder(img_path):
    ...
    # if path is a single image
        ...
        cleaning(img_path=img_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_name", type=str, default='celeb20')
    parser.add_argument("--img_ids", type=str, nargs='+', default=['ariana', 'bruce'])
    parser.add_argument("--attacks", type=str, nargs='+', default=['aspl'])
    ...
    from adavoc_package.RFB.data import VOC_300
    from adavoc_package.RFB.layers.functions import PriorBox
